...
static uint8_t _button(uint8_t *data) {
    ...
    switch (current_state) {
        ...
        default:
            PRINTLN_INFO("EVENT_BUTTON was triggered, but no action was taken based on the car's state (current_state=%d).", current_state);
            return U_SUCCESS;
    }
}
...
static uint8_t _dial(uint8_t *data) {
    ...
    switch (current_state) {
        ...
        /* Do nothing by default. */
        default:
            PRINTLN_INFO("EVENT_DIAL was triggered, but no action was taken based on the car's state (current_state=%d).", current_state);
            return U_SUCCESS;
    }
}
...
uint8_t statemachine_process_event(event_t event, uint8_t *data) {
    switch (event) {
        ...
        default: 
            PRINTLN_ERROR("Invalid event passed into function. (Event: %d)", event);
            return U_ERROR;
    }
}
