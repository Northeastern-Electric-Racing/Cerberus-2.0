...
void inbox_ethernet(ethernet_message_t *message) {
    switch(message->message_id) {
        ...
        case 0x03:
            ...
            break;
        default:
            DEBUG_PRINTLN("ERROR: Unknown Ethernet Message Recieved (Message ID: %X).", message->message_id);
            break;
    }
}
...
void inbox_can(can_msg_t *message) {
    switch(message->id) {
        ...
        case CANID_BMS_CELL_TEMPS:
            ...
            break;
        default:
            DEBUG_PRINTLN("ERROR: Unknown CAN Message Recieved (Message ID: %X).", message->id);
            break;
    }
}