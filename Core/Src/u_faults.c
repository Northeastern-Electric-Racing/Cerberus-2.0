...
static uint32_t severity_mask; // Mask that stores the severity configuration for each fault (0=NON_CRITICAL, 1=CRITICAL).
static volatile uint32_t fault_flags; // Each bit is a separate fault (0=Not Faulted, 1=Faulted).
...
_Static_assert(NUM_FAULTS <= 32, "This project does not (currently) support more than 64 faults."); // Ensures there aren't more than 32 faults.
