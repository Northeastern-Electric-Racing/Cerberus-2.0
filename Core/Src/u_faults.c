...
static const _metadata faults[] = {
	/* Critical Faults */
	[CAN_OUTGOING_FAULT] = { "CAN_OUTGOING_FAULT", CRITICAL,
				 .timeout = 5000 },
	[CAN_INCOMING_FAULT] = { "CAN_INCOMING_FAULT", CRITICAL,
				 .timeout = 5000 },
	[BMS_CAN_MONITOR_FAULT] = { "BMS_CAN_MONITOR_FAULT", CRITICAL,
				    .timeout = 5000 },
	[LIGHTNING_CAN_MONITOR_FAULT] = { "LIGHTNING_CAN_MONITOR_FAULT",
					  CRITICAL, .timeout = 5000 },
	[SHUTDOWN_FAULT] = { "SHUTDOWN_FAULT", CRITICAL, .timeout = 5000 },

	/* Non-critical Faults */
	[ONBOARD_TEMP_FAULT] = { "ONBOARD_TEMP_FAULT", NON_CRITICAL,
				 .timeout = 5000 },
	[IMU_ACCEL_FAULT] = { "IMU_ACCEL_FAULT", NON_CRITICAL,
			      .timeout = 5000 },
	[IMU_GYRO_FAULT] = { "IMU_GYRO_FAULT", NON_CRITICAL, .timeout = 5000 },
	[BSPD_PREFAULT] = { "BSPD_PREFAULT", NON_CRITICAL, .timeout = 5000 },
	[ONBOARD_BRAKE_OPEN_CIRCUIT_FAULT] = { "ONBOARD_BRAKE_OPEN_CIRCUIT_FAULT",
					       NON_CRITICAL, .timeout = 5000 },
	[ONBOARD_ACCEL_OPEN_CIRCUIT_FAULT] = { "ONBOARD_ACCEL_OPEN_CIRCUIT_FAULT",
					       NON_CRITICAL, .timeout = 5000 },
	[ONBOARD_BRAKE_SHORT_CIRCUIT_FAULT] = { "ONBOARD_BRAKE_SHORT_CIRCUIT_FAULT",
						NON_CRITICAL, .timeout = 5000 },
	[ONBOARD_ACCEL_SHORT_CIRCUIT_FAULT] = { "ONBOARD_ACCEL_SHORT_CIRCUIT_FAULT",
						NON_CRITICAL, .timeout = 5000 },
	[ONBOARD_PEDAL_DIFFERENCE_FAULT] = { "ONBOARD_PEDAL_DIFFERENCE_FAULT",
					     NON_CRITICAL, .timeout = 5000 },
	[RTDS_FAULT] = { "RTDS_FAULT", NON_CRITICAL, .timeout = 5000 },
	[LV_LOW_VOLTAGE_FAULT] = { "LV_LOW_VOLTS_FAULT", NON_CRITICAL,
				   .timeout = 5000 },
};

/* Fault Globals*/
static timer_t timers[NUM_FAULTS]; // Array of fault timers. One timer per fault.
static _Atomic uint32_t severity_mask =
	0; // Mask that stores the severity configuration for each fault (0=NON_CRITICAL, 1=CRITICAL).
static _Atomic uint32_t fault_flags =
	0; // Each bit is a separate fault (0=Not Faulted, 1=Faulted).
...
static void _timer_callback(ULONG args)
{
	...
	atomic_fetch_and(
		&fault_flags,
		~((uint32_t)(1
			     << fault_id))); // This is the _Atomic version of: fault_faults &= ~((uint32_t)(1 << fault_id));
	...
	for (int fault_id = 0; fault_id < NUM_FAULTS; fault_id++) {
		...
		if (faults[fault_id].severity == CRITICAL) {
			atomic_fetch_or(
				&severity_mask,
				((uint32_t)1
				 << fault_id)); // This is the _Atomic version of: severity_mask |= ((uint32_t)1 << fault_id);
		}
...
int trigger_fault(fault_t fault_id)
{
	...
	atomic_fetch_or(
		&fault_flags,
		(uint32_t)(1
			   << fault_id)); // This is the _Atomic version of: fault_flags |= (uint32_t)(1 << fault_id);
...
_Static_assert(
	NUM_FAULTS <= 32,
	"This project does not (currently) support more than 32 faults."); // Ensures there aren't more than 32 faults.
_Static_assert(
	sizeof(faults) / sizeof(faults[0]) == NUM_FAULTS,
	"Fault table size must match NUM_FAULTS. Make sure the fault table is consistent with the enum."); // Ensures the fault table size matches NUM_FAULTS.
